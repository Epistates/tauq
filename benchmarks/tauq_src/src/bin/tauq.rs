...
FORMAT OPTIONS:
    -O, --optimized         Comma-delimited for max token efficiency
    -U, --ultra             Comma-delimited + minified (smallest output)
...
EXAMPLES:
    ...
    # Convert JSON to Tauq (token-optimized, matches TOON density)
    tauq format data.json -O -o data.tqn

    # Convert JSON to Tauq (ultra-compact for minimum tokens)
