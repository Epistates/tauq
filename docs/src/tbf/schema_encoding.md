...
struct Data { /* ... */ }
// Uses compile-time schema generated by proc-macros
```

This section covers how to achieve the best compression through schema annotations.

## The TbfEncode and TbfDecode Traits

The `tbf_derive` crate provides macros that generate optimized encoding code:
...
For advanced **columnar** optimization, Tauq provides the `TableEncode` trait which can be implemented manually for maximum control:

```rust
use tauq::tbf::{TableEncode, TableSchema, FieldEncoding};
